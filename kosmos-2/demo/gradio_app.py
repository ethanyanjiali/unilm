...
Batch = namedtuple(
    "Batch",
    "ids src_tokens src_lengths constraints img_src_tokens img_gpt_input_mask")
    ...
    def convert_one_line(input_str):
        ...
        for i, segment in enumerate(segments):
            if segment.startswith('[image]'):
                ...
                image_tensor = square_transform(
                    self.args.input_resolution)(image)
                img_src_token.append(image_tensor)
                global global_image_tensor
                global_image_tensor = image_tensor
                token.extend([boi_id] +
                             list(range(4, image_feature_length + 4)) +
                             [eoi_id])
                ...
                special_tokens = [
                    self.source_dictionary[idx] for idx in range(
                        tokenizer.vocab_size(), len(self.source_dictionary))
                ]
                ...
                text_tokens = self.source_dictionary.encode_line(
                    encode_fn(segment), add_if_not_exist=False).tolist()

                text_tokens = text_tokens[:-1]  # </s> in token
                token.extend(text_tokens)
                img_gpt_input_mask.extend([0] *
                                          (len(text_tokens)))  # </s> in token
        token.append(eos_id)
...
def make_batches(lines, cfg, task, max_positions, encode_fn):
    ...
    if os.path.exists('data/sentencepiece.bpe.model'):
        ...
        tokenizer = None
    tokens, lengths, img_src_tokens, img_gpt_input_mask = get_interactive_tokens_and_lengths(
        task, lines, encode_fn, tokenizer)
...
def main(cfg: FairseqConfig):
    ...
    def generate_predictions(image_input, text_input, custom_input, do_sample,
                             sampling_topp, sampling_temperature):
        ...
        if text_input.lower() == 'brief':
            ...
            inputs = f"[image]{user_image_path}<tab>{custom_input}"
        ...
        for id_, src_tokens, hypos, info in sorted(results,
            ...
            for hypo in hypos[:min(len(hypos), cfg.generation.nbest)]:
                ...
                if global_image_path is not None:
                    basename = os.path.basename(global_image_path).split(
                        '.')[0]
                    vis_image = visualize_results_on_image(
                        global_image_path,
                        response_str,
                        task.args.location_bin_size,
                        f"output/store_vis_results/show_box_on_{basename}.jpg",
                        show=False)
                ...
                clean_response_str = ' '.join(
                    clean_response_str.split()).strip()
    ...
    with gr.Blocks(title="Kosmos-2", theme=gr.themes.Base()).queue() as demo:
        ...
        with gr.Row():
            with gr.Column():
                ...
                text_input = gr.Radio(["Brief", "Detailed", "Custom"],
                                      label="Description Type",
                                      value="Brief")
                custom_input = gr.Textbox(label="Custom Text", value="")
                do_sample = gr.Checkbox(
                    label="Enable Sampling",
                    info=
                    "(Please enable it before adjusting sampling parameters below)",
                    value=False)
                with gr.Accordion("Sampling parameters",
                                  open=False) as sampling_parameters:
                    sampling_topp = gr.Slider(minimum=0.1,
                                              maximum=1,
                                              step=0.01,
                                              value=0.9,
                                              label="Sampling: Top-P")
                    sampling_temperature = gr.Slider(
                        minimum=0.1,
                        maximum=1,
                        step=0.01,
                        value=0.7,
        ...
        run_button.click(fn=generate_predictions,
                         inputs=[
                             image_input, text_input, custom_input, do_sample,
                             sampling_topp, sampling_temperature
                         ],
...
def post_process_prediction(
    ...
    if align_dict is not None:
        hypo_str = utils.replace_unk(hypo_str, src_str, alignment, align_dict,
                                     tgt_dict.unk_string())
